  ...
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
	...
	if ((matches[1]:lower() == "sendplug" ) or (matches[1] == 'ارسال پلاگین' )) and matches[2] then
		...
		tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, plug, TiiGeRwmch, dl_cb, nil)
    end
...
if (matches[1]:lower() == 'TiiGeR' or matches[1] == 'تایکر') and is_sudio(msg) then
local info_text = 
[[
》MaTaDoR Cli v8.8
...
Info :]]..check_markdown(TiiGeRwmby)..[[

]]..check_markdown(TiiGeRwmch)..[[
]]
